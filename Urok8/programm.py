...
def del_contact():
    ...
    symbol = "\n"
    show_all()
    del_record = input("Введите номер строки для удаления: ")
    ...
    if exist_contact(del_record, ""):
        ...
        print(f'Удалена запись: {del_record}\n')
    else:
...
def search_contact():
    search_data = exist_contact(0, input("Ввод для поиска записи: "))
    if search_data:
